...
const LOCATIONS = gql`
  query locations($municipalityId: ID!) {
    municipality_locations(municipalityId: $municipalityId) {
      id
      location_name
    }
  }
...
export default function Location(props: { locationId: number; municipalityId: number; onClick: Function }) {
  const { data, loading } = useQuery<{ municipality_locations: Location[] }, { municipalityId: number }>(LOCATIONS, {
  ...
  return (
    <div className="py-6">
      {data?.municipality_locations?.map(location => (
        <span
          key={location.id}
