...
export default function Results(props: {electionType: string, electionId: number, districtId: number, municipalityId: number, round: number}) {
  const [results, setResults] = useState([] as Result[])
  const [selectedRound, setSelectedRound] = useState(props.round || 1)

  useEffect(() => {
    fetch(
      `/api/mayor_municipality_results?election_id=${props.electionId}&municipality_id=${props.municipalityId}&round=${selectedRound || 1}`)
      .then(response => response.json())
      .then(resultData => {
        setResults(resultData)
      })
    }, [props.municipalityId, selectedRound])
  ...
  return <>
    ...
    <Dropdown elements={[{id: 1, name: 'Тур I'}, {id: 2, name: 'Тур II'}]} selectedValue={selectedRound} onChange={(newRound: number) => navigate(`/elections/${props.electionId}/${props.districtId}/${props.municipalityId}/${newRound}`)} title="Тур" />
    ...
    <p>Гласували с опцията "Не подкрепям никого" {results[0].no_one} ({(results[0].no_one / results[0].total_voters * 100).toFixed(2)}%)</p>
  ...
  <Map election_id={props.electionId} municipality_id={props.municipalityId} round={selectedRound} />
  </>
}
