...
import MunicipalityRegion from "../municipalityRegion"
...
export default function Results(props: {electionType: string, electionId: number, districtId: number, municipalityId: number, regionId: number, round: number}) {
  const [results, setResults] = useState([] as Result[])

  useEffect(() => {
    fetch(
      `/api/mayor_municipality_region_results?election_id=${props.electionId}&municipality_id=${props.municipalityId}&region_id=${props.regionId}&round=${props.round}`)
      .then(response => response.json())
      .then(resultData => {
        setResults(resultData)
      })
    }, [props.municipalityId, props.regionId, props.round])


  if (!results.length) return <div>Loading...</div>
  return <>
    <MunicipalityRegion electionType={props.electionType} electionId={props.electionId} districtId={props.districtId} municipalityId={props.municipalityId} regionId={props.regionId} />

    <Dropdown elements={[{id: 1, name: 'Тур I'}, {id: 2, name: 'Тур II'}]} selectedValue={props.round} onChange={(newRound: number) => navigate(`/elections/mr/${props.electionId}/${props.districtId}/${props.municipalityId}/${props.regionId}/${newRound}`)} title="Тур" />
  ...
  <Map election_id={props.electionId} municipality_region_id={props.regionId} round={props.round} />
  </>
}
